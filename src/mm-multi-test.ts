...
async function restOrder(st: PS, side: "buy" | "sell", priceHuman: number, qtyHuman: number, orderType: "postOnly" | "immediateOrCancel"): Promise<any | null> {
  ...
  try {
    ...
    const rc: any = await withT(s.wait(1), 45_000, "order.wait");
    lastTxMs = Date.now();
    if (rc.status !== 1) { logT(st.symbol + side + "rev", `${st.symbol} ${side} ${orderType} tx revert`); return null; }
    return rc;
  } catch (e) { signer.reset(); logT(st.symbol + side + "senderr", `${st.symbol} ${side} ${orderType} НЕ прошёл: ${(e as Error).message.slice(0, 50)}`); return null; }
}
...
async function tickPair(st: PS) {
  ...
  if (invUSDso > dustEff) {
    // ЛОНГ → НЕМЕДЛЕННЫЙ ТЕЙКЕР-ФЛЭТ (железное правило: инвентарь не живёт, рынок не двигает баланс).
    // АНТИ-SELF-MATCH: пока на книге ЕСТЬ наш ордер, IOC-sell может влететь в наш же бид → «transaction execution
    // reverted» (это и завесило бота на 4ч). Снимаем ордера и ждём ОДИН тик (подтверждение отмены), только потом продаём.
    ...
    // как проверенный liquidate.ts: шлём ВЕСЬ остаток по bestBid. IOC частично исполнит при тонкой глубине,
    // остаток добьём следующим тиком (без estimateGas-гейта это больше не залипает).
    if (base > 0) {
    ...
    if (freeQuote >= clip * 0.5) {
      ...
      if (needRequote) {
        ...
        if (id !== null) { st.lastQuoteMid = mid; st.lastBidMs = Date.now(); log(`ORDER[${st.symbol}] maker-bid ${qty.toFixed(6)} @ ${bidPrice.toFixed(4)} (спред ${bookBps.toFixed(1)}бп)`); }
      }
    }
    ...
    if (!CFG.dryRun && idleOk && paceOk && bookBps <= CFG.churnMaxSpreadBps && freeQuote >= CFG.quoteFloorUSDso) {
      const buyQty = Math.min(clip / mid, askQty * 0.95);   // не глубже топ-аска
      const limitBuy = bestAsk * (1 + CFG.takerSlackBps * 1e-4);
      if (buyQty * mid > dustEff) {
        ...
        const fresh = await getOpenOrders(st);
        const stillBid = fresh.find(o => o.isBid);
        if (stillBid) await cancel(st, stillBid.id);
        const ok = await takerIOC(st, "buy", limitBuy, buyQty);
        if (ok) { st.lastChurnMs = Date.now(); log(`CHURN[${st.symbol}] IOC-buy ${buyQty.toFixed(6)} @~${bestAsk.toFixed(4)} (спред ${bookBps.toFixed(1)}бп)`); }
      }
    }
  }
}
...
async function main() {
  ...
  for (const symbol of CFG.pairs) {
    ...
    const st: PS = { symbol, mkt, pool, tickRaw: 0n, lotRaw: 0n, minQtyRaw: 0n, baseToken: "", quoteToken: "", approved: new Set(), lastQuoteMid: 0, lastNetBase: 0, cumVolume: 0, lastFillMs: Date.now(), lastChurnMs: 0, lastBidMs: 0, enabled: false };
    try {
